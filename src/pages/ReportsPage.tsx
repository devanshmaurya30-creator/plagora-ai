  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemFadeUpVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Report History</h1>
      ...
      {totalReportsCount > 0 && (
        <motion.div variants={itemFadeUpVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md">
            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">Total Reports</span>
            <div className="flex items-center gap-2 mt-1">
              <MagneticIcon maxOffset={2}><FileCheck className="w-5 h-5 text-blue-400" /></MagneticIcon>
              <span className="text-2xl font-extrabold text-white font-mono">{totalReportsCount}</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md">
            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">Avg. Similarity</span>
            <div className="flex items-center gap-2 mt-1">
              <MagneticIcon maxOffset={2}><Zap className="w-5 h-5 text-amber-400" /></MagneticIcon>
              <span className="text-2xl font-extrabold text-white font-mono">{avgSimilarity}%</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md">
            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">High Risk Scans</span>
            <div className="flex items-center gap-2 mt-1">
              <MagneticIcon maxOffset={2}><AlertTriangle className="w-5 h-5 text-rose-400" /></MagneticIcon>
              <span className="text-2xl font-extrabold text-white font-mono">{highRiskCount}</span>
            </div>
          </div>

          <div className="p-4 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md">
            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">Verified Web Sources</span>
            <div className="flex items-center gap-2 mt-1">
              <MagneticIcon maxOffset={2}><ShieldCheck className="w-5 h-5 text-emerald-400" /></MagneticIcon>
              <span className="text-2xl font-extrabold text-white font-mono">{verifiedSourcesCount}</span>
            </div>
          </div>
        </motion.div>
      )}

      {/* Filter Tabs & Search Controls (Requirement 26) */}
      <motion.div variants={itemFadeUpVariants} className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto py-1">
          {(['All', 'High Risk', 'Moderate', 'Low Risk'] as const).map((filter) => {
            ...
            return (
              ...
              >
                {/* Shared Sliding Active Background Indicator (Requirement 26) */}
                {isActive && (
                  <motion.div
        ...
        {/* Search Bar with Focus Glow (Requirement 27) */}
        <div className="relative max-w-xs w-full">
          ...
          <input
            ...
            className="w-full bg-white/[0.04] border border-white/15 rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-white/40 focus:bg-white/[0.07] focus:ring-2 focus:ring-white/20 transition-all duration-200"
          />
        </div>
      </motion.div>

      {/* Main Reports Table / List Container */}
      <motion.div variants={itemFadeUpVariants} className="rounded-3xl border border-white/15 bg-white/[0.02] backdrop-blur-xl overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="border-b border-white/10 bg-white/[0.03] text-slate-400 uppercase font-semibold">
              <tr>
                  ...
                  <tr key="empty-reports">
                    <td colSpan={6} className="p-12 text-center text-slate-500">
                      <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-400">No document reports match your search criteria.</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4 pl-6 font-semibold text-white">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl border border-white/15 bg-white/5 flex items-center justify-center text-blue-400 shrink-0 group-hover:scale-105 transition-transform">
                              <FileText className="w-4 h-4" />
                            </div>
                            <span className="truncate max-w-xs group-hover:text-white transition-colors">{item.documentName}</span>
                          </div>
                        </td>

                        {/* Report ID with Icon Morphing (Requirement 23) */}
                        <td className="p-4">
                          <button
                            onClick={() => handleCopyId(item.id)}
                            className="px-2 py-1 rounded text-[10px] uppercase font-mono bg-white/5 text-slate-400 border border-white/10 hover:border-white/30 hover:text-white transition-colors flex items-center gap-1.5"
                        ...
                        {/* Match Count */}
                        <td className="p-4 text-slate-400">{item.matches.length} matches</td>
      ...
      {/* PDF Generation Sequence Modal (Requirement 41) */}
      <AnimatePresence>
        {pdfGeneratingId && (
          <motion.div
          ...
          >
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="max-w-sm w-full rounded-3xl border border-white/20 bg-neutral-950 p-8 space-y-6 shadow-2xl text-center select-none"
            >
              <div className="w-14 h-14 rounded-2xl border border-white/20 bg-white/10 flex items-center justify-center mx-auto text-blue-400 shadow-lg">
                {pdfStep === 'Ready' ? (
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-bold text-white tracking-tight">Generating PDF Audit Report</h3>
                <p className="text-xs font-mono text-blue-400 uppercase tracking-widest">{pdfStep}...</p>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      {/* Delete Confirmation Modal (Requirement 38) */}
      <AnimatePresence>
        {deleteTargetId && (
          <motion.div
            variants={modalBackdropVariants}
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-white tracking-tight">Delete Report?</h3>
