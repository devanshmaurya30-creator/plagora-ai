        ...
        {isScanning && document ? (
          ...
          <motion.div key="scan-setup" className="space-y-8">
            <div className="border-b border-slate-200 dark:border-white/10 pb-6">
              <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">New <span className="font-serif-italic font-normal text-slate-800 dark:text-slate-200">Scan.</span></h1>
            ...
            {document && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="rounded-2xl border border-slate-200 dark:border-white/15 bg-white dark:bg-white/[0.03] backdrop-blur-xl p-6 md:p-8 space-y-8 shadow-xl"
              >
                {/* Stats Summary */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 text-xs">
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Filename</span>
                    <span className="font-semibold text-slate-900 dark:text-white truncate block">{document.fileName}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Word Count</span>
                    <span className="font-semibold text-slate-900 dark:text-white font-mono">
                      {document.wordCount.toLocaleString()}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Est. Scan Time</span>
                    <span className="font-semibold text-cyan-600 dark:text-blue-400">~{estimatedScanTime} sec</span>
                  </div>
                </div>
                      ...
                      <button
                        ...
                        className={`p-3.5 rounded-xl border text-left transition-all duration-200 ${
                          detectionDepth === depth
                            ? 'border-cyan-500 dark:border-white/40 bg-cyan-50 dark:bg-white/10 text-slate-900 dark:text-white shadow-sm'
                            : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.02] text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20'
                        }`}
                      >
                      ...
                      return (
                        <div
                          ...
                          className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer flex items-start gap-3 ${
                            isChecked
                              ? 'border-cyan-500/40 dark:border-white/25 bg-cyan-50/60 dark:bg-white/[0.05] text-slate-900 dark:text-white'
                              : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.01] text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20'
                          }`}
                        >
                          <div className="mt-0.5 text-cyan-600 dark:text-blue-400">
                            {isChecked ? (
                              <CheckSquare className="w-4 h-4 text-cyan-600 dark:text-white" />
                            ) : (
                              <Square className="w-4 h-4 text-slate-600" />
                            )}
                          </div>
                          <div>
                            <span className="text-xs font-semibold text-slate-900 dark:text-white block">{item.label}</span>
