...
export const SharedReportPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 min-h-screen select-none">
      {/* Top Banner: Read-Only Verification Badge */}
      <div className="p-3.5 rounded-2xl border border-blue-500/30 bg-blue-500/10 flex items-center justify-between text-xs text-blue-300">
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-blue-400" />
          <span>Verified Read-Only Public Report — Plagora AI Authenticated</span>
        </div>
        <span className="font-mono text-[10px] text-slate-400">ID: PLA-{analysis.id.slice(0, 8).toUpperCase()}</span>
      </div>
        ...
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 p-1 rounded-2xl">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
              activeTab === 'Matches' ? 'bg-white/15 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            Matches & Proof
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
              activeTab === 'Insights' ? 'bg-white/15 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            Deep Insights
          </button>
      ...
      {activeTab === 'Insights' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-6">
            ...
            {selectedMatch && (
              <div className="rounded-3xl border border-white/20 bg-neutral-950/90 p-6 space-y-4 shadow-2xl">
