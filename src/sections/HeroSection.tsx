...
export const HeroSection: React.FC = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Hero mouse parallax (max 6px offset)
    setParallaxOffset({
      x: dx * 6,
      y: dy * 6,
    });
  ...
  return (
    ...
    >
      {/* Hero Decorative Layer Mouse Parallax (Requirement 16) */}
      <motion.div
        animate={{ x: parallaxOffset.x * -1.2, y: parallaxOffset.y * -1.2 }}
        transition={{ type: 'spring', stiffness: 150, damping: 25 }}
        className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-gradient-to-tr from-blue-600/10 via-purple-500/10 to-transparent blur-[140px] rounded-full pointer-events-none"
      />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex justify-center">
            <Badge>AI-POWERED PLAGIARISM DETECTION</Badge>
          </motion.div>
          ...
          <motion.p
            variants={itemVariants}
            className="max-w-2xl mx-auto text-base sm:text-lg text-slate-400 font-normal leading-relaxed"
          >
            Analyze documents for copied, paraphrased, and semantically similar content with
            advanced AI-powered detection.
          </motion.p>

          {/* CTA Buttons with Magnetic & Click Physics */}
          <motion.div
            variants={itemVariants}
          ...
          {/* Bottom Statistics Cards with 3D Tilt & Hover Lift (Requirements 9, 10, 11) */}
          <motion.div
            variants={itemVariants}
            className="pt-16 grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto border-t border-white/[0.08]"
          >
            <TiltCard maxRotate={2.5} liftY={-6} className="p-6 text-center">
