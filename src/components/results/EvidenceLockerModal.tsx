    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md select-none">
        <motion.div
          ...
          className="max-w-3xl w-full max-h-[85vh] rounded-3xl border border-white/20 bg-neutral-950 p-6 md:p-8 space-y-6 shadow-2xl text-slate-100 flex flex-col justify-between relative"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-white tracking-tight uppercase">EVIDENCE LOCKER & COLLECTIONS</h3>
                <p className="text-[11px] text-slate-400">Persisted Evidence Vault & Research Records</p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between gap-2 overflow-x-auto py-1">
            <div className="flex items-center gap-2">
              ...
              {collections.map((col) => (
                <span
                  key={col.id}
                  className="px-2.5 py-1 rounded-xl text-xs font-semibold bg-purple-500/15 text-purple-300 border border-purple-400/30 flex items-center gap-1"
                >
            ...
            <button
              ...
              className="text-xs font-semibold text-purple-400 hover:text-white flex items-center gap-1 shrink-0 cursor-pointer"
            >
          ...
          {showAddCol && (
            <div className="flex items-center gap-2 p-2 rounded-xl bg-neutral-900 border border-white/10">
              <input
                type="text"
          ...
          <div className="flex-1 overflow-y-auto pr-1 space-y-3 max-h-[380px]">
            {items.length === 0 ? (
              <div className="p-8 text-center text-xs text-slate-500 border border-white/10 rounded-2xl bg-white/[0.01]">
              ...
              items.map((item) => {
                ...
                return (
                  <div key={item.id} className="p-4 rounded-2xl border border-white/10 bg-white/[0.02] space-y-2 relative">
                    <div className="flex items-center justify-between text-xs font-mono">
                      <span className="font-bold text-white">{item.sourceTitle}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-amber-400 font-bold">{item.similarity}% similarity</span>
                        <button
                    ...
                    <p className="text-xs text-slate-300 italic bg-black/40 p-2.5 rounded-xl border border-white/5 line-clamp-2">
                      "{item.originalText}"
                    </p>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1">
                      ...
                      {safeUrl && (
                        <a href={safeUrl} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-white flex items-center gap-1">
